...
```bash
uv sync --project benchmarks/envs/esmc
uv sync --project benchmarks/envs/esm2
uv sync --project benchmarks/envs/saprot
```
...
```bash
uv run --project benchmarks/envs/esmc \
  python scripts/extract_esmc_embeddings.py

uv run --project benchmarks/envs/esm2 \
  python scripts/extract_esm2_embeddings.py

uv run --project benchmarks/envs/saprot \
...
The split file in `benchmarks/splits/` should be committed so every model and
